...
# Keep the existing ROI selection code the same
...
# YOLO-specific functions and classes
class YOLODetector:
    def __init__(self, weights_path, config_path, names_path, conf_threshold=0.3, nms_threshold=0.4):
        """
        Initialize YOLO detector with model configuration, weights, and class names
        
        Args:
            weights_path: Path to the YOLO weights file (.weights)
            config_path: Path to the YOLO config file (.cfg)
            names_path: Path to the class names file (.names)
        ...
        # Get network output layer names
        ...
        # Map from COCO dataset indices to our class indices
        self.coco_to_class_map = {0: 0, 1: 1, 2: 2, 3: 3, 5: 4, 7: 5}  # person, bicycle, car, motorcycle, bus, truck
        
        print(f"YOLO detector initialized with {len(self.classes)} classes")
    ...
    def detect(self, frame):
        ...
        for output in outputs:
            for detection in output:
                ...
                if confidence > self.conf_threshold:
                    ...
                    # relative to the image size (0.0 to 1.0)
        ...
        detections = []
        for i in indices:
            # Handle different versions of OpenCV that return indices differently
            if isinstance(i, list) or isinstance(i, np.ndarray):
                i = i[0]  # For OpenCV < 4.5.4
...
def main():
    ...
    try:
        ...
        detector = YOLODetector(weights_path, config_path, names_path, 
